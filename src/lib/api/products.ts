...
export const productsApi = {
  ...
  getById: async (storeId: string, id: string, includeMedia = false): Promise<Product> => {
    const params = new URLSearchParams();
  ...
  getBySlug: async (storeId: string, slug: string, includeMedia = false): Promise<Product> => {
    const params = new URLSearchParams();
    if (includeMedia) params.append('includeMedia', 'true');
    const query = params.toString() ? `?${params.toString()}` : '';
